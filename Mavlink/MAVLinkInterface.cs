...
4. Try a diffrent usb port\n\n" +
            ...
            while (true)
            {
                ...
                if (buffer.Length > 5)
                {
                    if (buffer.msgid == (byte)MAVLINK_MSG_ID.PARAM_VALUE)
                    {
                        ...
                        {
                            /*
                            GCS.instance.BeginInvoke((Action) delegate
                            {
        ...
        */
        ...
        {
            return GCS.comPort.doCommand(MAV_CMD.DO_MOTOR_TEST, (float)motor, (float)(byte)thr_type,
        ...
        public void setGuidedModeWP(byte sysid, byte compid, Locationwp gotohere, bool setguidedmode = true)
        {
            ...
            try
            {
                ...
                if (MAVlist[sysid, compid].cs.firmware == GCS.Firmwares.ArduPlane)
                {
                ...
                else
                {
                    setPositionTargetGlobalInt((byte)sysid, (byte)compid,
                        ...
                        gotohere.lat, gotohere.lng, GCS.comPort.MAV.GuidedMode.z, 0, 0, 0, 0, 0);
                }
            }
            catch (Exception ex)
            {
        ...
        public void setDigicamControl(bool shot)
        {
            ...
            GCS.comPort.doCommand(MAV_CMD.DO_DIGICAM_CONTROL, 0, 0, 0, 0, 1, 0, 0);
                ...
                if ((message.header == 'U' || message.header == 0xfe || message.header == 0xfd) && buffer.Length >= message.payloadlength)
                {
                    ...
                    lock (objlock)
                    {
                        ...
                        if (msgid == (byte)MAVLINK_MSG_ID.ADSB_VEHICLE)
                        {
                            ...
                            if (GCS.instance.adsbPlanes.ContainsKey(id))
                            {
                                // update existing
                                ((adsb.PointLatLngAltHdg)GCS.instance.adsbPlanes[id]).Lat = adsb.lat / 1e7;
                                ((adsb.PointLatLngAltHdg)GCS.instance.adsbPlanes[id]).Lng = adsb.lon / 1e7;
                                ((adsb.PointLatLngAltHdg)GCS.instance.adsbPlanes[id]).Alt = adsb.altitude / 1000.0;
                                ((adsb.PointLatLngAltHdg)GCS.instance.adsbPlanes[id]).Heading = adsb.heading * 0.01f;
                                ((adsb.PointLatLngAltHdg)GCS.instance.adsbPlanes[id]).Time = DateTime.Now;
                                ((adsb.PointLatLngAltHdg)GCS.instance.adsbPlanes[id]).CallSign = ASCIIEncoding.ASCII.GetString(adsb.callsign);
                            }
                            else
                            {
                                // create new plane
                                GCS.instance.adsbPlanes[id] =
                        ...
                        if (msgid == (byte)MAVLINK_MSG_ID.COLLISION)
                        {
                            ...
                            if (GCS.instance.adsbPlanes.ContainsKey(id))
                            {
                                ((adsb.PointLatLngAltHdg)GCS.instance.adsbPlanes[id]).ThreatLevel = threat_level;
                            }
                        }
                    }
                    ...
                    if (msgid == (byte)MAVLINK_MSG_ID.STATUSTEXT) // status text
                    {
                        ...
                        if (printit)
                        {
                            ...
                            /*if (GCS.speechEngine != null &&
                                GCS.speechEngine.IsReady &&
                                Settings.Instance["speechenable"] != null &&
                                Settings.Instance["speechenable"].ToString() == "True")
                            {
                                if (speechenabled)
                                    GCS.speechEngine.SpeakAsync(logdata);
                            }*/
                        }
                    }
        ...
        public void setAPType(byte sysid, byte compid)
        {
            ...
            switch (MAVlist[sysid, compid].apname)
            {
                case MAV_AUTOPILOT.ARDUPILOTMEGA:
                    switch (MAVlist[sysid, compid].aptype)
                    {
                        case MAV_TYPE.FIXED_WING:
                            MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.ArduPlane;
                            break;
                        case MAV_TYPE.QUADROTOR:
                            MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.ArduCopter2;
                            break;
                        ...
                        case MAV_TYPE.GROUND_ROVER:
                            MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.ArduRover;
                            break;
                        case MAV_TYPE.SUBMARINE:
                            MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.ArduSub;
                            break;
                        case MAV_TYPE.ANTENNA_TRACKER:
                            MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.ArduTracker;
                            break;
                        default:
                ...
                case MAV_AUTOPILOT.GENERIC:
                    ...
                    {
                        case MAV_TYPE.FIXED_WING:
                            MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.Ateryx;
                            break;
                    }
                    break;
                case MAV_AUTOPILOT.PX4:
                    MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.PX4;
                    break;
                default:
                    switch (MAVlist[sysid, compid].aptype)
                    {
                        case MAV_TYPE.GIMBAL: // storm32 - name 83
                            MAVlist[sysid, compid].cs.firmware = GCS.Firmwares.Gymbal;
                            break;
                    }
                    break;
            }
