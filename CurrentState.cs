    ...
    public class CurrentState : ICloneable
    {
        ...
        public float ch3percent
        {
            get
            {
                ...
                try
                {
                    if (GCS.comPort.MAV.param.ContainsKey("RC3_MIN") &&
                        GCS.comPort.MAV.param.ContainsKey("RC3_MAX"))
                    {
                        return
                            (int)
                                (((ch3out - GCS.comPort.MAV.param["RC3_MIN"].Value) /
                                  (GCS.comPort.MAV.param["RC3_MAX"].Value - GCS.comPort.MAV.param["RC3_MIN"].Value)) *
                                 100);
                    }
                    else
                    {
        ...
        public float crit_AOA
        {
            get
            {
                try
                {
                    if (GCS.comPort.MAV.param.ContainsKey("AOA_CRIT"))
                    {
                        return
                            (int)
                                (GCS.comPort.MAV.param["AOA_CRIT"].Value);
                    }
                    else
                    {
                        return 25;
        ...
        public float GeoFenceDist
        {
            get
            {
                try
                {
                    ...
                    var R = 6371e3;
                    // close loop
                    var list = GCS.comPort.MAV.fencepoints.ToList();
                    if (list.Count > 0)
                    {
        ...
        // current firmware
        public GCS.Firmwares firmware = GCS.Firmwares.ArduCopter2;
        ...
        {
            get { return (GCS.comPort.BaseStream.IsOpen || GCS.comPort.logreadmode); }
        }
        ...
        {
            UpdateCurrentSettings(bs, false, GCS.comPort, GCS.comPort.MAV);
        }
                ...
                if (DateTime.Now > lastupdate.AddMilliseconds(50) || updatenow) // 20 hz
                {
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        else
                        {
                            ...
                            /*if (oldmode != mode && GCS.speechEnable && GCS.comPort.MAV.cs == this &&
                                Settings.Instance.GetBoolean("speechmodeenabled"))
                            {
                                GCS.speechEngine.SpeakAsync(Common.speechConversion("" + Settings.Instance["speechmode"]));
                            }*/
                        }
                    }
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        /*if (oldwp != wpno && GCS.speechEnable && GCS.comPort.MAV.cs == this &&
                            Settings.Instance.GetBoolean("speechwaypointenabled"))
                        {
                            GCS.speechEngine.SpeakAsync(Common.speechConversion("" + Settings.Instance["speechwaypoint"]));
                        }*/
