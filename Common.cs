...
using GCS.Utilities.Attributes;
using GMap.NET;
...
namespace GCS
{
    public class Common
    {
        ...
        public static List<KeyValuePair<int, string>> getModesList(GCS.Firmwares firmware)
        {
            //log.Info("getModesList Called");

            if (firmware == GCS.Firmwares.PX4)
            {
                /*
union px4_custom_mode {
    ...
    float data_float;
            ...
            else if (firmware == GCS.Firmwares.ArduPlane)
            {
            ...
            else if (firmware == GCS.Firmwares.Ateryx)
            {
                var flightModes = Utilities.ParameterMetaDataRepository.GetParameterOptionsInt("FLTMODE1",
                    firmware.ToString()); //same as apm
                return flightModes;
            }
            else if (firmware == GCS.Firmwares.ArduCopter2)
            {
                var flightModes = Utilities.ParameterMetaDataRepository.GetParameterOptionsInt("FLTMODE1",
                    firmware.ToString());
                return flightModes;
            }
            else if (firmware == GCS.Firmwares.ArduRover)
            {
                var flightModes = Utilities.ParameterMetaDataRepository.GetParameterOptionsInt("MODE1",
                    firmware.ToString());
                return flightModes;
            }
            else if (firmware == GCS.Firmwares.ArduTracker)
            {
                var temp = new List<KeyValuePair<int, string>>();
        ...
        public static Form LoadingBox(string title, string promptText)
        {
            ...
            System.ComponentModel.ComponentResourceManager resources =
                new System.ComponentModel.ComponentResourceManager(typeof(GCS));
        ...
        {
            if (GCS.comPort.MAV.cs.wpno == 0)
            {
                input = input.Replace("{wpn}", "Home");
            }
            else
            {
                input = input.Replace("{wpn}", GCS.comPort.MAV.cs.wpno.ToString());
            }

            input = input.Replace("{asp}", GCS.comPort.MAV.cs.airspeed.ToString("0"));

            input = input.Replace("{alt}", GCS.comPort.MAV.cs.alt.ToString("0"));

            input = input.Replace("{wpa}", GCS.comPort.MAV.cs.targetalt.ToString("0"));

            input = input.Replace("{gsp}", GCS.comPort.MAV.cs.groundspeed.ToString("0"));

            input = input.Replace("{mode}", GCS.comPort.MAV.cs.mode.ToString());

            input = input.Replace("{batv}", GCS.comPort.MAV.cs.battery_voltage.ToString("0.00"));

            input = input.Replace("{batp}", (GCS.comPort.MAV.cs.battery_remaining).ToString("0"));

            input = input.Replace("{vsp}", (GCS.comPort.MAV.cs.verticalspeed).ToString("0.0"));

            input = input.Replace("{curr}", (GCS.comPort.MAV.cs.current).ToString("0.0"));

            input = input.Replace("{hdop}", (GCS.comPort.MAV.cs.gpshdop).ToString("0.00"));

            input = input.Replace("{satcount}", (GCS.comPort.MAV.cs.satcount).ToString("0"));

            input = input.Replace("{rssi}", (GCS.comPort.MAV.cs.rssi).ToString("0"));

            input = input.Replace("{disthome}", (GCS.comPort.MAV.cs.DistToHome).ToString("0"));

            input = input.Replace("{timeinair}",
                (new TimeSpan(0, 0, 0, (int)GCS.comPort.MAV.cs.timeInAir)).ToString());

            try
            {
                object thisBoxed = GCS.comPort.MAV.cs;
        ...
        public static GMapMarker getMAVMarker(MAVState MAV)
        {
           ...
           /*if (MAV.aptype == MAVLink.MAV_TYPE.FIXED_WING)
            ...
            else if (MAV.cs.firmware == GCS.Firmwares.ArduTracker)
            {
                return (new GMapMarkerAntennaTracker(portlocation, MAV.cs.yaw,
                    MAV.cs.target_bearing));
            }*/
            //else if (MAV.cs.firmware == GCS.Firmwares.ArduCopter2 || MAV.aptype == MAVLink.MAV_TYPE.QUADROTOR)
             if (MAV.cs.firmware == GCS.Firmwares.ArduCopter2 || MAV.aptype == MAVLink.MAV_TYPE.QUADROTOR)
            {
